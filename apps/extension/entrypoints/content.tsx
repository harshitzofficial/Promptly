...
const App = () => {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      const cacheRes = await browser.runtime.sendMessage({ action: 'checkCache', prompt: text, ratio: compressionRatio });
      if (cacheRes?.hit && cacheRes.data?.optimizedPrompt) {
        ...
        if (sessionId) {
          await fetch(`${BACKEND_URL}/api/log-cache`, {
            method: 'POST',
      ...
      // ── 2. Check for custom provider key (OpenAI / Anthropic) ────────────
      const port = browser.runtime.connect({ name: 'providerStream' });
      
      const providerRes: any = await new Promise((resolve, reject) => {
        let streamed = '';
        let started = false;
        
        port.onMessage.addListener((msg) => {
          if (msg.type === 'error') {
            if (msg.useBackend) resolve({ success: false, useBackend: true });
            else { alert(msg.error); resolve({ success: false }); }
          } else if (msg.type === 'chunk') {
            if (!started) { setEditableText(target, ''); started = true; }
            streamed += msg.text;
            setEditableText(target, streamed);
          } else if (msg.type === 'done') {
            resolve({ success: true, optimizedPrompt: msg.fullText, method: msg.method });
          }
        });
        
        port.postMessage({ prompt: text, ratio: compressionRatio });
      });

      if (providerRes?.success && providerRes.optimizedPrompt) {
        const origTokens = text.trim().split(/\s+/).length * 1.3 | 0; // rough estimate
        const optTokens = providerRes.optimizedPrompt.trim().split(/\s+/).length * 1.3 | 0;
        const saved = Math.max(0, origTokens - optTokens);
        const method = providerRes.method;

        // Log to backend to calculate pricing based on current platform config
        const sessionRes = await browser.runtime.sendMessage({ action: 'getSession' });
        const sessionId = sessionRes?.data?.sessionId ?? '';
        let actualCostSaved = 0;

        if (sessionId) {
          const logRes = await fetch(`${BACKEND_URL}/api/log-custom`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              sessionId, prompt: text, compressed: providerRes.optimizedPrompt,
              originalTokens: origTokens, optimizedTokens: optTokens, method
            })
          }).then(r => r.json()).catch(() => ({}));
          if (logRes.costSaved) actualCostSaved = logRes.costSaved;
        }

        browser.runtime.sendMessage({ action: 'setCache', prompt: text, ratio: compressionRatio, result: {
          optimizedPrompt: providerRes.optimizedPrompt, tokensSaved: saved, percentSaved: origTokens > 0 ? Math.round((saved / origTokens) * 100) : 0,
          originalTokens: origTokens, optimizedTokens: optTokens, method, costSaved: actualCostSaved
        }});

        setStats({ saved, percent: origTokens > 0 ? Math.round((saved / origTokens) * 100) : 0, method, costSaved: actualCostSaved });
        setLastFromCache(false);
        setTimeout(() => setStats(null), 4000);
        return;
      }

      if (providerRes && !providerRes.success && !providerRes.useBackend) {
        setIsOptimizing(false);
        return;
      }

      // ── 3. Cache miss + no custom provider — stream from Gemini backend ───
      const sessionRes = await browser.runtime.sendMessage({ action: 'getSession' });
      const sessionId = sessionRes?.data?.sessionId ?? '';
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.type === 'chunk') {
              ...
              browser.runtime.sendMessage({ action: 'setCache', prompt: text, ratio: compressionRatio, result: data });
    ...
    <div className="fixed top-8 right-8 z-[100000] bg-[#1a1a2e] text-[#e5e5e5] px-4 py-3 rounded-lg shadow-xl border border-[#333] text-[13px] pointer-events-auto flex flex-col gap-1">
      ...
      <div className="flex items-center gap-2 text-[11px]">
        {lastFromCache
          ...
          : stats.method === 'gemini-agent'
            ? <span className="text-[#a78bfa]">✨ Curated by AI Agent</span>
            : stats.method === 'openai'
            ? <span className="text-[#10a37f]">🟢 Curated by GPT-4o mini</span>
            : stats.method === 'anthropic'
            ? <span className="text-[#c96442]">🟠 Curated by Claude Haiku</span>
    ...
    >
      ...
      <div className="bg-[#0f0f11]/80 backdrop-blur-2xl border border-r-0 border-white/10 p-5 rounded-bl-2xl shadow-[0_8px_32px_rgba(0,0,0,0.7)] flex flex-col gap-5 text-[12px] text-[#a0a0a0] font-sans w-72">
        ...
        <div className="flex items-center justify-center gap-2 pt-3 border-t border-white/5">
          <span className="text-[#444] text-[9px] uppercase tracking-widest font-sans font-bold">Shortcut</span>
          <div className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 bg-white/[0.07] border border-white/10 rounded text-[10px] font-mono text-[#888] shadow-inner leading-none">Ctrl</kbd>
            <span className="text-[#444] text-[9px]">+</span>
            <kbd className="px-1.5 py-0.5 bg-white/[0.07] border border-white/10 rounded text-[10px] font-mono text-[#888] shadow-inner leading-none">Shift</kbd>
            <span className="text-[#444] text-[9px]">+</span>
            <kbd className="px-1.5 py-0.5 bg-white/[0.07] border border-white/10 rounded text-[10px] font-mono text-[#888] shadow-inner leading-none">O</kbd>
          </div>
        </div>
